document.addEventListener("DOMContentLoaded", function () {
    ...
    async function fetchCategories() {
        try {
            const categories = await fetchWithRetry("http://localhost:3000/api/categories");
    ...
    function openEditOverlay(staff) {
        ...
        if (staff.image) {
            editStaffImagePreview.src = `http://localhost:3000/${staff.image}`;
    ...
    editStaffImageInput.addEventListener("change", function () {
        ...
        if (file) {
            if (!validateImageType(file)) {
                showAlert("Only images in JPEG, JPG, or PNG formats are allowed");
                editStaffImageInput.value = "";
                editStaffImagePreview.src = existingImagePath ? `http://localhost:3000/${existingImagePath}` : "";
    ...
    addStaffForm.addEventListener("submit", async function (e) {
        ...
        try {
            const data = await fetchWithRetry("http://localhost:3000/api/add-staff", {
                method: "POST",
                body: formData,
            });
    ...
    editStaffForm.addEventListener("submit", async function (e) {
        ...
        try {
            const data = await fetchWithRetry(`http://localhost:3000/api/update-staff/${currentStaffId}`, {
                method: "PUT",
                body: formData,
            });
    ...
    async function fetchStaff(searchQuery = "") {
        ...
        try {
            const staff = await fetchWithRetry("http://localhost:3000/api/staff");
            ...
            if (filteredStaff.length === 0) {
                    ...
                    row.innerHTML = `
                        ...
                        <td>
                            <div class="staff-name">
                                ${s.image ? `<img src="http://localhost:3000/${s.image}" alt="${s.name}" class="staff-image">` : ''}
                                <span>${s.name}</span>
                            </div>
                        </td>
                        <td>${s.phone || 'N/A'}</td>
                ...
                document.querySelectorAll(".edit-button").forEach(button => {
                    button.addEventListener("click", function () {
                        ...
                        fetchWithRetry(`http://localhost:3000/api/staff/${staffId}`)
                            .then(staff => {
                ...
                document.querySelectorAll(".delete-button").forEach(button => {
                    button.addEventListener("click", function () {
                        ...
                        showAlert("Are you sure you want to delete this staff?", () => {
                            fetchWithRetry(`http://localhost:3000/api/delete-staff/${staffId}`, {
                                method: "DELETE",
                            })
                                .then(data => {
    ...
    document.getElementById("resetPasswordBtn").addEventListener("click", function () {
        ...
        if (confirmed) {
            fetchWithRetry(`http://localhost:3000/api/reset-staff-password/${currentStaffId}`, {
                method: "POST",
            })
                .then(data => {
