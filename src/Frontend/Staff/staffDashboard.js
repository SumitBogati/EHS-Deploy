document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function fetchBookings() {
        try {
            const response = await fetch(`http://localhost:3000/api/bookings/staff/${userId}`);
    ...
    editBookingForm.addEventListener('submit', async (event) => {
        ...
        try {
            ...
            if (newDate && newTimeSlot) {
                const rescheduleResponse = await fetch(`http://localhost:3000/api/bookings/${bookingId}/reschedule`, {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json'
                    },
