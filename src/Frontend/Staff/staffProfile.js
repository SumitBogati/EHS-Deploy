...
async function fetchStaffData(userId) {
    try {
        ...
        if (response.ok) {
            ...
            document.getElementById('image-preview').src = staffData.image ? `http://localhost:3000/${staffData.image}` : '/Image/default-staff.png';
...
submitPasswordChange.addEventListener("click", async () => {
    ...
    try {
        const response = await fetch("http://localhost:3000/api/update-staff-password", {
            method: "POST",
            headers: {
